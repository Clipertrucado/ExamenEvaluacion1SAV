...
export function listaPrestamos() {
    ...
    const divListaPrestamos = document.getElementById('texto');
    
    divListaPrestamos.innerHTML =  " "
    let ulLista = document.createElement('ul');
    ulLista.className = "list-group";

    arrayPrestamos.forEach(prestamo => {
        let li = document.createElement('li');
        li.textContent = prestamo.id + ", " + prestamo.titulo + ", " + prestamo.autor + ", " + prestamo.editorial + ", " + prestamo.tema + ", " + prestamo.prestado;
    ...
    divListaPrestamos.appendChild(ulLista);
}
    

export function imgPrestamos() {
    // devuelve la imagen de préstamos
    const imgPrestamos = document.getElementById('img');

    imgPrestamos.src = imagenes.prestamos;
}

export function listaSocios() {
    const divListaTexto = document.getElementById('texto');
    
    divListaTexto.innerHTML =  " "
    let ulLista = document.createElement('div');
    ulLista.className = "list-group";
    
    arraySocios.forEach(socio => {
        let li = document.createElement('li');
        li.textContent = socio.idSocio + ", " + socio.nombre + ", " + socio.mail;
        li.className = "list-group-item";
        ulLista.appendChild(li);
    });
    ...
    divListaTexto.appendChild(ulLista);
}

export function imgSocios() {

    const imgSocio = document.getElementById('img');

    imgSocio.src = imagenes.socios;
}